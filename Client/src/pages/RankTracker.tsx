...
export default function RankTracker() {
    ...
    const handleAdd = async (e: React.SubmitEvent) => {
        ...
        if (!newKeyword.trim() || !newUrl.trim()) {
            setAddError("Please enter both a keyword and a website URL.");
            return;
        }

        setAdding(true);
        try {
            const response = await api.post("/api/rankings", { keyword: newKeyword, url: newUrl });
            setKeywords((items) => [response.data.ranking, ...items]);
            setNewKeyword("");
            setNewUrl("");
            setShowAddModal(false);
        } catch (error) {
            const apiError = error as { response?: { data?: { message?: string } }; message?: string };
            setAddError(apiError.response?.data?.message || apiError.message || "Unable to track this keyword.");
        } finally {
            setAdding(false);
        }
    };
    ...
    const handleRefresh = async (id: string) => {
        setRefreshing(id);
        try {
            const response = await api.post(`/api/rankings/${id}/refresh`);
            setKeywords((items) => items.map((item) => (item._id === id ? response.data.ranking : item)));
        } catch (error) {
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeleting(id);
        try {
            await api.delete(`/api/rankings/${id}`);
            setKeywords((items) => items.filter((item) => item._id !== id));
        } catch (error) {
    ...
    const handleToggle = async (id: string) => {
        const keyword = keywords.find((item) => item._id === id);
        if (!keyword) return;

        try {
            const response = await api.patch(`/api/rankings/${id}`, { active: !keyword.active });
            setKeywords((items) => items.map((item) => (item._id === id ? response.data.ranking : item)));
        } catch (error) {
    ...
    const biggestGain = keywords.reduce((best, item) => Math.max(best, item.positionChange || 0), 0);
        ...
        <div className="min-h-screen pt-16 md:pt-24 bg-background">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent mb-2">AI SERP monitor</p>
                        <h1 className="text-2xl sm:text-4xl font-semibold text-foreground">
                            Rank <span className="gradient-text">Tracker</span>
                        </h1>
                        <p className="text-muted-foreground text-sm mt-1">Track Google positions, competitor pressure, and ranking movement in one place.</p>
                    </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                    {[
                        { label: "Active keywords", value: activeKeywords, tone: "text-primary" },
                        { label: "Ranked", value: rankedKeywords, tone: "text-accent" },
                        { label: "Top 10", value: topTenKeywords, tone: "text-success" },
                        { label: "Biggest gain", value: `+${biggestGain}`, tone: "text-emerald-400" },
                    ].map((stat) => (
                    ...
                    <div className="bg-background border border-border rounded-2xl p-6 w-full max-w-md">
                        ...
                        <form onSubmit={handleAdd} className="space-y-4">
                            ...
                            <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 text-xs text-muted-foreground">
                                <p>Tip: Rank Pilot will check your keyword, estimate your website position, and keep the result in this tracker.</p>
                            </div>
