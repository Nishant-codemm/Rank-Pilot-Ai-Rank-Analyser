...
export default function Navbar() {
    ...
    return (
        <nav className="fixed top-0 w-full bg-background/80 backdrop-blur-xl border-b border-border/70 z-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6">
                <div className="flex items-center justify-between h-16">
                    {/* Logo */}
                    <Link to="/" className="flex items-center gap-2 group">
                        <span className="size-9 rounded-xl bg-primary text-background flex items-center justify-center">
                            <ChartNoAxesColumnIcon size={20} />
                        </span>
                        <span className="text-xl font-semibold tracking-tight text-foreground">Rank Pilot</span>
                    </Link>
                        ...
                        {user ? (
                            <>
                                <div className="flex items-center gap-2 px-2 py-1.5 rounded-full border border-border bg-card text-sm">
                                    ...
                                    <span className="px-2 py-0.5 rounded-full text-[10px] font-medium uppercase bg-accent/10 border border-accent/15 text-accent">{user.plan || "free"}</span>
                                </div>
